void piki (double* amplitudes, double* frequences, double*& x, double*& y, int*& l, int* q) //x, y - АиЧ текущего трэка; l - число пиков текущего трэка; q - кол-во элементов функции
{int i, j, t, flag, Num;
Num=0;

for (i=0; i<=q; i++) //идем по значениям функции, где q - максимальное кол-во значений в массивах АЧХ
 if (amplitudes[i]>501)
 {
  Num++;
  while ((amplitudes[i]>501)&&(i<=q))	//пропускаем пик
   i++;
 }
...
bool analysis (double* x, double* y, double* p, double* r, int n, int l)	//x, y - АиЧ текущего трэка; p, r - АиЧ трэка из базы
